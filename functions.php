 ...
 *
 * {
 * "url": "‌https://dataapi.pingpongx.com/withdraw/detail",
 * "data": {
 * "tx_id": "W20201029140335353099262",
 * "app_id": "9001035",
 * "app_secret": "O30F0QRZUUTWJWZY",
 * "sign": "5DD1EFF6654501BC0853B08006075A0B"
 * },
 * "header": [
...
function proxy()
{
    ...
    $url = isset($inputArr['url']) ? (string)$inputArr['url'] : '';
    $data = isset($inputArr['data']) ? (array)$inputArr['data'] : '';
    $header = isset($inputArr['header']) ? (array)$inputArr['header'] : [];
    $tokenType = isset($inputArr['tokenType']) ? $inputArr['tokenType'] : '';
    $timeout = isset($inputArr['tokenType']) ? $inputArr['timeout'] : 60;
    if (empty($url)) {
        exit('empty url');
    }
    echo "[request url]:\n";
    ...
    $response = http_curl($url, $data, $timeout, $tokenType, $header);
    echo "\n\n[response data]:\n";
...
function createDir($path)
{
    ...
    //$path = iconv("UTF-8", "GBK", $path); // window中文乱码，需转码
    if (empty($path)) {
        return false;
    }
    if (is_dir($path)) {
        ...
        //第三个参数是“true”表示递归模式创建多级目录，iconv防止中文目录乱码
...
{
    //$path = iconv("UTF-8", "GBK", $filePath); // window中文乱码，需转码
    if (empty($filePath)) {
        return false;
    }
...
function json($arr, $return = false)
{
...
//判断是否为命令行模式:    php_sapi_name() //cli: 命令模式    cgi-fcgi: cgi模式     apache2handler: apache模式
function isCli()
{
...
//获取某个月第一天/最后一天
function getMonthFirstDayAndLastDay($date)
{
...
//utf-8 转中文 gbk 输出信息
function printMsg($message, $line = PHP_EOL)
{
    if (is_array($message)) {
        ...
        printMsg(get_object_vars($message));
    } else {
        echo iconv('utf-8', 'gbk', $message), $line;
    }
}


function dcTestSql($data, $type = 'mysql')
{
    $url = 'http://dc.yibainetwork.com:86/index.php/Test/testSql';
    $time = time();
    $sign = md5('testSql' . $time . '78ab23572c0091d70850de6f1ce030b4');
    $header[] = 'Content-Type: application/json; charset=utf-8';
    $header[] = 'time: ' . $time;
    $header[] = 'sign: ' . $sign;
    $res = curlPost($url, $data, 60, $header);
    return $res;
}

function hwcTestSql($data, $type = 'mysql')
{
    $url = 'http://hwcadmin.yibainetwork.com:30023/index.php/Test/testSql';
    $time = time();
    $sign = md5('testSql' . $time . '78ab23572c0091d70850de6f1ce030b4');
    $header[] = 'Content-Type: application/json; charset=utf-8';
    $header[] = 'time: ' . $time;
...
//修改海外仓映射库存，修改具体excel数据, 列要重新核对，修改代码再执行
function skuMapModify($runSql = 0)
{

    $file = 'C:\Users\Administrator\Downloads\yb_order_sku_mapping(3)映射1.csv';
    ...
    foreach ($data as $key => $item) {
        ...
        $oldNum = (int)$item[4];
        $item[6] = trim($item[6]);
        $modifyNum = (int)str_replace('数据中心', '', $item[6]);
        //$num = $oldNum + $modifyNum;
		$num = $modifyNum;
        //ddd("{$oldNum} + {$modifyNum} = {$num}");
        $sqlArr[] = "update yb_order_sku_mapping set available_stock='{$num}' where id={$id};";
    }
    ddd($sqlArr);
    if ($runSql) {
        foreach ($sqlArr as $sql) {
            ...
            $data['type'] = 'mysql';
            $data['sql'] = $sql;
            $res = hwcTestSql(json_encode($data));
            ddd($res);
        }
    }
}

function getDcmConnection($dbName = 'db_base', $env = 'test')
{
    switch ($env) {
        case 'test':
            $hostname = '192.168.71.92';
            break;
        case 'production':
            $hostname = '';        //124.71.85.224 连不上
            break;
        default:
            $hostname = '192.168.71.224';
            break;
    }
    $database = [
        'db_base' => 'yibai_dcm_base',
        'db_system' => 'yibai_dcm_system',
        'db_order' => 'yibai_dcm_order',
        'db_amazon' => 'yibai_dcm_amazon',
        'db_platform_goods' => 'yibai_dcm_platform_goods',
        'db_agent' => 'yibai_dcm_agent',
        'db_logistics' => 'yibai_tms_logistics',
        'db_message' => 'yibai_dcm_message',
        'db_product' => 'yibai_dcm_product',
        'db_finance' => 'yibai_dcm_finance',
    ];
    $config = [
        'host' => $hostname,
        'user' => 'hujuncai',
        'password' => 'HJC@2021*9j',
        'db' => $database[$dbName],
    ];
...
function parseHtml()
{
    $file = 'C:\Users\Administrator\Desktop\bookmarks_2021_12_10.html';
...
//        echo 'length:' . $nodeList->length;
    for ($i = 0; $i < $nodeList->length; $i++) {
        $item = $nodeList->item($i);
        $item->hasAttribute('href') && $href = $item->getAttribute("href");
        $temp = [
            'value' => $item->nodeValue,
            'href' => $href,
        ];
        $a = "<a href='{$href}'>{$item->nodeValue}</a>";
        $a = $item->nodeValue . ':' . $href;
        $data[] = $a;
    }
...
//查看亚马逊报告原文档信息：view-source:http://test.cc/test.php 打开
function checkReportDocumentDetail()
{
	$str = <<<EOF
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201818999.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201816250.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201815335.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201813299.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201811485.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201810344.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201809573.txt
/amazon_reports/20230208/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230208_201807101.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220901648.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220810235.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220719576.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220640270.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220539700.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220503947.txt
/amazon_reports/20230207/GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE/report_425_15579_20230207_220413750.txt
EOF;
	$uploadUrl = 'http://dcmpubhk.yibainetwork.com/upload';
	$arr = explode(PHP_EOL, $str);
	$content = [];
	foreach($arr as $saveUrl){
		$url = $uploadUrl.$saveUrl;
		echo $url;
		echo PHP_EOL;
	...
	//file_put_contents('report.txt',implode("\r\n",$content));
	
}

//延迟重试请求函数，超过次数终止
function setTimeTick($tyrTimes = '') 
{
	$tyrTimes = $maxTimes = 3;
	$api_res = [];
	while ($tyrTimes) {
		sleep($maxTimes - $tyrTimes);
		//do something
		$api_res = [];
		if (isset($api_res['access_token']) && !empty($api_res['access_token'])) {
			break;
		}
		$tyrTimes--;
	}
